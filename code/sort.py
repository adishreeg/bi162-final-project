...
def sort(filename, sorter):
    recording = si.BinaryRecordingExtractor(
        [filename], 
        sampling_frequency=info['rate_hz'],
        num_chan=2,
        dtype=float, gain_to_uV=1, offset_to_uV=0)
    recording.set_dummy_probe_from_locations(np.zeros([2,2]))
    sorting = ss.run_sorter(sorter, recording, docker_image=True)
    torch.save(f'{sorter}_{filename}', sorting)


filename = '160516.dat'
sorter = 'kilosort3'
sort(filename, sorter)
